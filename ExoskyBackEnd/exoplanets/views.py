...
from .services import get_exoplanets_data, save_exoplanets_data, get_random_exoplanets_data, get_exoplanets_by_id
...
class ExoplanetsByIdDataView(APIView):
    def post(self, request):
        try:
            data = get_exoplanets_by_id(request.data.get('pl_name'))
            return Response(data)
        except Exception as e:
            return Response({"error": str(e)}, status=500)